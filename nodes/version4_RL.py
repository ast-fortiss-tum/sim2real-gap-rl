...
class CustomDonkeyEnvWrapper(gym.Wrapper):
    def __init__(self, env):
        super(CustomDonkeyEnvWrapper, self).__init__(env)
        print("Environment created.")

        # Modify the observation space to be a Dict with 'image' and 'state'
        ...
        # Modify the action space to steering only
    ...
    def reset(self):
        ...
        info = {'speed': 0.0, 'cte': 0.0}  # Initialize info
...
# Make gym env using gym.make
env = gym.make(env_list[1], conf=env_config)

# Wrap the env with your custom wrapper
env = CustomDonkeyEnvWrapper(env)
