...
def linear_decay(epoch, start_decay_epoch=75, total_epochs=150):
    if epoch < start_decay_epoch:
        return 1.0
    return 1.0 - float(epoch - start_decay_epoch) / (total_epochs - start_decay_epoch)

def half_decay(epoch):
    if epoch == 0:
        return 1.0
    else:
        return 0.5
...
def train_cyclegan(
    ...
    # Schedulers
    # -----------------------
    scheduler_G   = LambdaLR(optimizer_G,   lr_lambda=lambda e: linear_decay(epoch = e, start_decay_epoch = 2, total_epochs=epochs))
    scheduler_D_A = LambdaLR(optimizer_D_A, lr_lambda=lambda e: linear_decay(epoch = e, start_decay_epoch = 2, total_epochs=epochs))
    scheduler_D_B = LambdaLR(optimizer_D_B, lr_lambda=lambda e: linear_decay(epoch = e, start_decay_epoch = 2, total_epochs=epochs))

    #scheduler_G = LambdaLR(optimizer_G, lr_lambda=lambda epoch: step_decay(epoch, initial_lr=1.0, decay_factor=0.5))
    #scheduler_D_A = LambdaLR(optimizer_D_A, lr_lambda=lambda epoch: step_decay(epoch, initial_lr=1.0, decay_factor=0.5))
    #scheduler_D_B = LambdaLR(optimizer_D_B, lr_lambda=lambda epoch: step_decay(epoch, initial_lr=1.0, decay_factor=0.5))
    ...
    for epoch in range(epochs):
        ...
        print(f"Epoch {epoch+1}/{epochs}, Current LR = {current_lr:.9f}")
        ...
        for i in range(loader_size):
            ...
            # (Here, we log once per epoch for brevity)

            # Suppose 'img_tensor' is your [C, 128, 128] tensor, already denormalized if necessary.
            # We want to resize it to [C, 240, 320].
            un_transform = transforms.Compose([
                transforms.ToPILImage(),  # Tensor -> PIL Image
                transforms.Resize((240, 320), interpolation=InterpolationMode.BICUBIC),
                transforms.ToTensor()     # PIL -> Tensor
            ])

            #resized_realA = un_transform(realA)
            #resized_realB = un_transform(realB)
            #resized_fakeA = un_transform(fakeA)
            #resized_fakeB = un_transform(fakeB)
            ...
            if (i == 0) and (epoch % 1 == 0):
                ...
                #writer.add_images("Real/SimR", resized_realA, epoch)
                #writer.add_images("Real/CarR", resized_realB, epoch)
                #writer.add_images("Fake/SimR", resized_fakeA, epoch)
                #writer.add_images("Fake/CarR", resized_fakeB, epoch)
        ...
        # -----------------------
        if epoch % 1 == 0:
            torch.save(netG_AB.state_dict(), os.path.join(save_dir, f"Car_netG_AB_epoch_{epoch+1}.pth"))
            torch.save(netG_BA.state_dict(), os.path.join(save_dir, f"Car_netG_BA_epoch_{epoch+1}.pth"))
            torch.save(netD_A.state_dict(), os.path.join(save_dir, f"Car_netD_A_epoch_{epoch+1}.pth"))
            torch.save(netD_B.state_dict(), os.path.join(save_dir, f"Car_netD_B_epoch_{epoch+1}.pth"))
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="CycleGAN Monet2Photo Example with Model Saving, Param Counting, and TensorBoard Logging"
    )
    parser.add_argument("--data_root", type=str, default="sim2Car",
    ...
    parser.add_argument("--log_dir", type=str, default="runs/cyclegan_car",
