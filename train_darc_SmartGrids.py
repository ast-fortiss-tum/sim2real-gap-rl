...
def parse_args():
    ...
    parser.add_argument('--lin_src', type=int, default=None,
                        help='For non-broken experiments, which environment is linear: 1 for source, 0 for target (mandatory when broken==0)')
    ...
    parser.add_argument('--break_src', type=int, default=None,
                        help='For broken experiments, which environment to break: 1 for source, 0 for target (mandatory when broken==1)')
...
def main():
    ...
    if args.broken == 0:
        ...
        if args.lin_src == 1:
            source_env = get_simple_linear_env()
            if args.variety_name == 's':
                target_env = get_new_soc_env(args.degree)
            elif args.variety_name == 'c':
                target_env = get_new_charge_env(args.degree)
            elif args.variety_name == 'd':
                target_env = get_new_discharge_env(args.degree)
            elif args.variety_name.startswith('v'):
                target_env = get_new_all_eff_env(args.degree)
            elif args.variety_name == 'lc':
                target_env = get_new_limited_capacity_env(args.capacity, 1.5)
            elif args.variety_name == 'lp':
                target_env = get_new_limited_plim_env(3, args.p_lim)
            else:
                raise ValueError("Unknown variety name: " + args.variety_name)
        else:
            target_env = get_simple_linear_env()
            if args.variety_name == 's':
                source_env = get_new_soc_env(args.degree)
            elif args.variety_name == 'c':
                source_env = get_new_charge_env(args.degree)
            elif args.variety_name == 'd':
                source_env = get_new_discharge_env(args.degree)
            elif args.variety_name.startswith('v'):
                source_env = get_new_all_eff_env(args.degree)
            elif args.variety_name == 'lc':
                source_env = get_new_limited_capacity_env(args.capacity, 1.5)
            elif args.variety_name == 'lp':
                source_env = get_new_limited_plim_env(3, args.p_lim)
            else:
                raise ValueError("Unknown variety name: " + args.variety_name)
    else:
        # Two-house setting (broken environment).
        if args.break_src == 1:
            source_env = get_twoHouses_env(damaged_battery=True)
            target_env = get_twoHouses_env(damaged_battery=False)
        else:
            source_env = get_twoHouses_env(damaged_battery=False)
            target_env = get_twoHouses_env(damaged_battery=True)
    ...
    # When broken==0, we only use one-house soc input (e.g. one value); when broken==1, two-house so two values.
