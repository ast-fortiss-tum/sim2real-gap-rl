...
"""
Autor: Cristian Cubides
This script creates an animation of vehicle poses overlaid on lane data.
...
The lanes are drawn with specific colors and styles, and the vehicle's position
is represented by a marker that changes color based on its status.
"""
...
def animate_pose(ax, poses, times, lanes, going_times, going_flags,
    ...
    """
    Create an animation on ax using pose data that now include time.
    
    For each frame:
      - Interpolated (x, y, t) data is used.
      - The current pose is tested for being "on track" (inside the area defined by lane0 and lane1).
      - The current "going" flag (from the going JSON) is obtained based on the pose's time.
      - The animated marker is colored as:
            * Black if not going (external intervention),
            * Green if going and on track,
            * Red if going and off-track.
      - The trace is drawn as segments:
            * If the going flag is False → segment colored black.
            * If True → segment colored green if both endpoints are on track; else red.
            
    Additionally, when the animation reaches the final frame, it computes and displays
    the "Porcentage on-track" (only counting samples when going is True).
    """
    ...
    def init():
        ...
        return point, trace_collection, percentage_text
    
    def update(frame):
        x, y, t = interp_poses[frame]
        # Check if the current pose is "on track".
        ...
        # When on the final frame, compute and display the percentage.
        if frame == len(interp_poses) - 1:
            # Consider only samples with going_flag == True.
...
def main():
    parser = argparse.ArgumentParser(
        description="Animation with lane coloring and selective black segments based on 'going' status."
    )
    # File with lanes, poses, and pose time stamps.
    parser.add_argument('--dataset', type=str, default='dataset_final',
                help="Name of main folder of dataset (e.g., 'dataset_final').")
    ...
    parser.add_argument('--name', type=str, default='1',
                        help="Name of experiments file for poses and lanes (without extension) (e.g., 1).")
    ...
    poses_file_path = f'./results_reality/{dataset}/{folder}/{name}_poses.json'
    going_path = f'./results_reality/{dataset}/{folder}/{name}_moving.json'
    ...
    ax.set_title("Lanes and Pose Animation with Going Intervals")
    ax.grid(True)
    ...
    ani = animate_pose(ax, poses, poses_time, lanes, going_times, going_list,
                       interval=interval, steps_per_segment=10, subsample=int(interval))
