...
Usage examples
--------------
python compare_algos.py                         # plot every algorithm
python compare_algos.py --exclude ContSAC_FT    # drop one
python compare_algos.py --exclude ContSAC_FT ContSAC_STR
"""
...
drop_first_k  = 0                             # episodes to skip
...
# regex *after* filtering
algo_pat  = re.compile(rf"^({'|'.join(map(re.escape, algos))})", re.IGNORECASE)
deg_pat   = re.compile(r"degree_([-+]?\d*\.?\d+)", re.IGNORECASE)

# prefix → TensorBoard tag (adjust if your logging differs!)
tag_map = {
    "DARC":    "Target Env/Rewards",   # slash
    "ContSAC": "Target/Env/Rewards",   # space
}
...
for root, _, files in os.walk(folder_path):
    ...
    if algo.lower().startswith("darc"):
        arr = arr[:-1]                   # drop last step for DARC
    curve = np.cumsum(arr)
...
# ───────────────────── helper: pad 2‑d array ──────────────────────
def pad_to_len(curve_list, length):
    return np.vstack(
        [np.pad(c, (0, length - len(c)), constant_values=np.nan) for c in curve_list]
    )
...
algos_present = list(stat_algo.keys())             # after filtering + availability
cmap = plt.cm.tab10(np.linspace(0, 1, len(algos_present)))

# ───────────────────────── figure 1: curves ───────────────────────
fig1, ax1 = plt.subplots(figsize=(6.2, 3.4))
for c, algo in zip(cmap, algos_present):
    med, q1, q3, n = stat_algo[algo]
    ep = np.arange(1, len(med) + 1)
    ax1.plot(ep, med, "-", color=c, lw=1.4, label=f"{algo} (n={n})")
    ax1.fill_between(ep, q1, q3, color=c, alpha=0.25)
...
fig1.legend(*ax1.get_legend_handles_labels(), fontsize=6, loc="upper center",
...
degrees_present = sorted(deg_algo_vals.keys())
bar_width = 0.8 / max(1, len(algos_present))       # fit ≤0.8 total

# ─────────── figure 3: mean ± std bar‑chart (safe) ────────────────
...
# ─────────── figure 4: one box‑plot file per degree ───────────────
for deg in degrees_present:
    data, labels, colors = [], [], []
    for algo, color in zip(algos_present, cmap):
        vals = deg_algo_vals[deg].get(algo, [])
        if vals:
            data.append(vals); labels.append(algo); colors.append(color)
    if not data:
        continue
