...
class GailContSAC_SRC_Classifier:
    def __init__(self, policy_config, value_config, env,target_env, device, expert,expert_model, log_dir="latest_runs",running_mean = None,
                ...
                reward_type = 1,train_c = 1,model_size=64,dis_update_freq = 50, gp = False):
        self.device = device
        self.gamma = gamma
        ...
        sa_config = {
            ...
            "architecture": [{"name": "linear1", "size": model_size},
                             {"name": "linear2", "size": 2}],
        ...
        self.dm_opt = Adam(self.dm.parameters(), lr=0.001)
...
#             print(s_states.shape, s_actions.shape)
            ...
            if self.train_c:
                ...
                sa_logits = self.expert_model.sa_classifier(sa_inputs.to(self.device)+ gen_noise(self.noise_scale, sa_inputs, self.device))
                sas_logits = self.expert_model.sas_adv_classifier(sas_inputs.to(self.device)+ gen_noise(self.noise_scale, sas_inputs, self.device))
...
#             w_t = torch.clamp(w_t,self.clamp_reward_lower_bound,self.clamp_reward_upper_bound)
    ...
    def train_step(self, states, actions, rewards, next_states, done_masks,weight = None):
        if weight is None:
            weight = 1
        else:
            weight = torch.tensor(weight).reshape((len(weight),1)).to(self.device)
...
#         q_loss = weight * (functional.mse_loss(pred_q1, expected_q) + functional.mse_loss(pred_q2, expected_q))
        q_loss = weight * ((pred_q1-expected_q)**2 + (pred_q2- expected_q)**2)
        q_loss = q_loss.mean()
        ...
        policy_loss = weight * (self.alpha * s_log_prob - self.twin_q(states, s_action)[0])
    ...
    def save_model(self, folder_name):
        folder_name = folder_name + '_imitationWalker' 
...
#                         w, logD, _ = self.expert_reward(s,a,r,s_)
                        wc = wc*w
#                         print(wc)
                        r_hat = 0
                        if self.reward_type == 10:
                        ...
                        if self.reward_type in [12,13]:
                            train_info = self.train_step(s, a, r, s_, d,w)
                        else:
                            train_info = self.train_step(s, a, r, s_, d)
                        dis_step +=1
                        self.writer.add_train_step_info(train_info, i)
                        if self.update_dis_flag % self.dis_update_freq == 0:
                            self.update_dis_flag = 0
...
#             state, action, r, next_state, d = self.ilmemory.sample()
            index = np.random.randint(0, high=len(self.expert)-1, size=2048)
            ...
            if self.gp:
                eta = torch.FloatTensor(g_o.shape[0],1).uniform_(0,1).to(self.device)
                interpolated = eta * g_o_input + ((1 - eta) * expert_state_actions)

                prob_interpolated = self.discrim_net(interpolated)
                gradients = autograd.grad(outputs=prob_interpolated, inputs=interpolated,
                           grad_outputs=torch.ones(
                ...
                discrim_loss =  -torch.mean(wt.unsqueeze(1) * torch.log(g_o))+ \
                    self.discrim_criterion(e_o, torch.zeros((index.shape[0], 1), device=self.device)) + grad_penalty
            else:
                discrim_loss =  -torch.mean(wt.unsqueeze(1) * torch.log(g_o))+ \
    ...
    def expert_reward(self,state,action,reward, next_state):
        ...
        with torch.no_grad():
            if len(state.shape) == 1:
                ...
                state = torch.tensor(state,dtype= dtype).to(self.device)
                next_state = torch.tensor(next_state,dtype= dtype).to(self.device)
                action = torch.tensor(action,dtype= dtype).to(self.device)         
                wt = self.cal_wt(state,action,next_state).cpu()
                wt = torch.clamp(wt,self.clamp_reward_lower_bound,self.clamp_reward_upper_bound)

                return wt,-torch.log(1e-8 + self.discrim_net(state_action  + gen_noise(self.noise_scale, state_action, self.device))),reward
