...
class DARC(ContSAC):
    ...
    def simulate_env(self, game_count, env_name, deterministic):
        ...
        while not done:
            ...
            next_state, reward, done, _, _ = env.step(action)    # Correct the action due to safetylayer set up in controller and in system.
            ...
            # Flatten the numpy arrays: for each array, iterate over its values.
            ver_actions_values = [value for arr in ver_actions_values for value in arr]

            # Flatten the dict_values: for each dict_values, convert to list and iterate.
...
class DARC_two(ContSAC):
    ...
    def train_step(self, s_states, s_actions, s_rewards, s_next_states, s_done_masks, *args):
        ...
        with torch.no_grad():
            # Build inputs including both state[100] and state[226]
            sa_inputs = torch.cat([
        ...
        # Classifier inputs now use both state indices.
        s_sa_inputs = torch.cat([
            torch.cat([s_states[:, 100].unsqueeze(1), s_states[:, 226].unsqueeze(1)], dim=1),
            s_actions[:, 0].unsqueeze(1)
        ], dim=1)
    ...
    def simulate_env(self, game_count, env_name, deterministic):
        ...
        while not done:
            if game_count <= self.warmup_games:
                ...
                action = self.get_action(state, deterministic)   # Get primitive action from policy
            next_state, reward, done, _, _ = env.step(action)    # Correct the action due to safetylayer setup
            ...
            # Logging verified actions/penalties and the selected state indices.
            ver_actions = env.env.sys.verified_actions
            ver_penalties = env.env.sys.verification_penalties
            ...
            print("VERIFIED ACTIONS VALUES: ", ver_actions_values)
            print("VERIFIED PENALTIES VALUES: ", ver_penalties_values)
            # Print both index 100 and 226 from state and next_state.
            print("SOC: ", state[100], state[226])
            print("next_SOC: ", next_state[100], next_state[226])
            print("UNVERIFIED ACTION: ", action)
            print("+++++++++++++++++++++++++++++++++")

            time.sleep(3)
            ##############
            memory.add(state, ver_actions_values, reward, next_state, done_mask=1.0 if n_steps == env._max_episode_steps - 1 else 0.0)
            # memory.add(state, action, reward, next_state, done_mask)
