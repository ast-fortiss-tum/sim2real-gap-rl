...
#             wt = torch.clamp(wt,0.1,10)
#             wt = torch.clamp(wt,0.05,20)
#             wt = torch.clamp(wt,0.01,100)
            # wt = torch.clamp(wt,0.001,1000)
            # wt = 1

            r = -math.log(1e-8 + self.discrim_net(state_action  + gen_noise(self.noise_scale, state_action, self.device))[0].item()) * wt.item()
            # r = -math.log(1e-8 + self.discrim_net(state_action  + gen_noise(self.noise_scale, state_action, self.device))[0].item()) 

            self.wt_list.append(wt.item())
            self.expert_reward_list.append(r)
            
            # r = torch.clamp(wt,0.01,100).item()
            return r
