...
#             sa_logits = self.expert_model.sa_classifier(sa_inputs.to(self.device))
#             sas_logits = self.expert_model.sas_adv_classifier(sas_inputs.to(self.device))
...
#             w_t = torch.clamp(w_t,self.clamp_reward_lower_bound,self.clamp_reward_upper_bound)
            ...
            # w_t = torch.clamp(w_t,0.1,10)
            # w_t = torch.clamp(w_t,0.05,20)
            # w_t = torch.clamp(w_t,0.01,100)
            # w_t = torch.clamp(w_t,0.001,1000)
    ...
    def train_classifier(self):
        ...
        s_states, s_actions, s_rewards, s_next_states, s_done_masks = self.memory.sample()
    ...
    def train(self, num_games, deterministic=False):
        ...
        for i in range(num_games):
            ...
            while not done:
                ...
                total_reward += reward
                reward = self.expert_reward(state,action,reward,next_state)
            ...
            if i >= self.warmup_games:
                ...
                if i % self.n_games_til_train == 0:
                    for _ in range(n_steps * self.n_updates_per_train):
                        ...
                        if self.update_dis_flag % self.update_freq == 0:
                            self.update_dis_flag = 0
    ...
    def update_discriminator(self):
        for _ in range(1):
            state, action, r, next_state, d = self.memory.sample()
...
#                 reweight log D
                r = -math.log(1e-8 + self.discrim_net(state_action  + gen_noise(self.noise_scale, state_action, self.device))[0].item()) * wt.item()
            elif self.reward_type == 2:
#                 DR method with log D as DM:
                r = wt.item() * reward - (1-wt.item()) * math.log(1e-8 + self.discrim_net(state_action  + gen_noise(self.noise_scale, state_action, self.device))[0].item())
            elif self.reward_type == 3:
#             pure ips
                r = wt.item() * reward
            self.wt_list.append(wt.item())
            self.expert_reward_list.append(r)
