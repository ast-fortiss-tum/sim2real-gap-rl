...
class DARC(ContSAC):
    ...
    def add_obs_noise_percent(self, obs):
        """Adds Gaussian noise as a relative error to the observation element at index 100.
        
        The noise is applied as a multiplicative factor:
            new_value = original_value * (1 + noise)
        where noise ~ N(0, self.noise_scale).
        
        If the value at index 100 is 0, the noise is added as an absolute error.
        """
        obs_noisy = np.copy(obs)
        if obs_noisy[100] != 0:
            # Apply relative (multiplicative) noise
            noise = np.random.normal(0, self.noise_scale)
            obs_noisy[100] *= (1 + noise)
        else:
            # For zero, add absolute noise instead to avoid degenerate scaling
    ...
    def train_step(self, s_states, s_actions, s_rewards, s_next_states, s_done_masks, *args):
        ...
        with torch.no_grad():
            # Instead of denoising, we use the raw value at index 100.
            s_recovered = s_states[:, 100].unsqueeze(1)      # (batch, 1)
            s_next_recovered = s_next_states[:, 100].unsqueeze(1)
            t_recovered = t_states[:, 100].unsqueeze(1)
            
            # Build classifier inputs using raw values.
            ...
            # Inject noise into the inputs using gen_noise.
            ...
            # Compute delta reward using the noisy logits.
        ...
        # Now compute classifier logits with injected noise.
    ...
    def simulate_env(self, game_count, env_name, deterministic):
        ...
        # For target, set up the recording lists.
        if env_name == "target":
            # Record the value before any noise is added.
            real_val = state[100]
            # Create a noisy version of the initial state.
            noisy_state = self.add_obs_noise(state)
            noisy_val = noisy_state[100]
            # Initialize the accumulated sequence with the noisy input.
            accumulated_sequence = [noisy_val]
            # Run the denoiser on the accumulated sequence.
            ...
            # Replace the feature at index 100 with the recovered value.
        ...
        while not done:
            ...
            if env_name == "target":
                # Record the original value from the environment.
                real_val = next_state[100]
                
                # Add noise only for target:
                noisy_next_state = self.add_obs_noise(next_state)
                noisy_val = noisy_next_state[100]

                # Append the noisy value to the accumulated sequence.
                accumulated_sequence.append(noisy_val)

                # Build the accumulated sequence tensor and recover the denoised value.
                acc_tensor = torch.tensor(accumulated_sequence, dtype=torch.float32).unsqueeze(0).unsqueeze(-1).to(self.device)
                with torch.no_grad():
                    denoised_seq, _ = self.denoiser.forward_online(acc_tensor)
                recovered_val = denoised_seq[0, -1, :].cpu().numpy()[0]

                # Replace the feature in next_state with its recovered version.
            ...
            # For the source environment, no noise or recording is needed.
        ...
        # At the end of the episode, if we're using the target environment, plot the comparison.
        if False:
            if env_name == "target":
                import matplotlib.pyplot as plt
                plt.figure(figsize=(10, 6))
                plt.plot(real_values, label="Real Input", marker='o')
                plt.plot(noisy_values, label="Noisy Input", marker='x')
                plt.plot(recovered_values, label="Recovered", marker='s')
                plt.xlabel("Episode Step")
                plt.ylabel("Feature Value at Index 100")
                plt.title("Comparison of Real, Noisy, and Recovered Inputs")
                plt.legend()
                plt.tight_layout()
                plt.show()
...
#-------------------------------------------------
# DARC_two is a subclass of DARC that uses the same architecture but with different configurations.
# It is designed to be used with a different set of parameters or configurations.
#-------------------------------------------------
import numpy as np
import torch
from torch import nn
...
# -------------------------------------------
# Modified DARC class using online denoiser with accumulating buffer.
# Now extracting state indices 100 and 226 instead of just 100.
# -------------------------------------------
class DARC_two(ContSAC):
    def __init__(self, policy_config, value_config, sa_config, sas_config, source_env,
        ...
        super(DARC_two, self).__init__(policy_config, value_config, source_env, device, log_dir,
                                     running_mean, noise_scale, bias, memory_size, warmup_games, batch_size,
                                     lr, gamma, tau, alpha, ent_adj, target_update_interval,
                                     n_games_til_train, n_updates_per_train, max_steps)
        ...
        # Load the already trained online autoencoder as your denoiser.
        # Change input_dim from 1 to 2 to support the two extracted indices.
        self.denoiser = OnlineDenoisingAutoencoder(input_dim=1, proj_dim=16, lstm_hidden_dim=32, num_layers=1).to(self.device)
        self.denoiser.load_state_dict(torch.load("best_online_denoising_autoencoder.pth", map_location=self.device, weights_only=True))
        print("Denoiser loaded successfully.")
        self.denoiser.eval()
    ...
    def add_obs_noise_percent(self, obs):
        """Adds Gaussian noise as a relative error to the observation elements at indices 100 and 226.
        
        The noise is applied as a multiplicative factor:
            new_value = original_value * (1 + noise)
        where noise ~ N(0, self.noise_scale).
        
        If the value at an index is 0, the noise is added as an absolute error.
        """
        obs_noisy = np.copy(obs)
        for idx in [100, 226]:
            if obs_noisy[idx] != 0:
    ...
    def train_step(self, s_states, s_actions, s_rewards, s_next_states, s_done_masks, *args):
        ...
        with torch.no_grad():
            # Instead of denoising, we use the raw values at indices 100 and 226.
            s_recovered = s_states[:, [100, 226]]      # (batch, 2)
            s_next_recovered = s_next_states[:, [100, 226]]
            t_recovered = t_states[:, [100, 226]]
            
            # Build classifier inputs using raw values.
            sa_inputs = torch.cat([s_recovered, s_actions], dim=1)  # shape: (batch, 3)
            sas_inputs = torch.cat([s_recovered, s_actions, s_next_recovered], dim=1)  # shape: (batch, 5)
    ...
    def simulate_env(self, game_count, env_name, deterministic):
        ...
        if env_name == "target":
            # --- Initialization for denoising indices 100 and 226 separately ---
            ...
            # Replace the features with the recovered values.
            state[100] = recovered_val_100
            state[226] = recovered_val_226
            
            # Initialize lists for plotting separately for each index.
        ...
        while not done:
            ...
            if env_name == "target":
                # --- Process denoising separately for indices 100 and 226 ---
                # Record original values.
                ...
                # Append to the accumulated sequences.
        ...
        # --- (Optional) Plot the recorded sequences for comparison. ---
        if False:
            if env_name == "target":
                import matplotlib.pyplot as plt
                plt.figure(figsize=(10, 6))
                plt.plot(real_values_100, label="Real Input Index 100", marker='o')
                plt.plot(noisy_values_100, label="Noisy Input Index 100", marker='x')
                plt.plot(recovered_values_100, label="Recovered Index 100", marker='s')
                plt.plot(real_values_226, label="Real Input Index 226", marker='o')
                plt.plot(noisy_values_226, label="Noisy Input Index 226", marker='x')
                plt.plot(recovered_values_226, label="Recovered Index 226", marker='s')
                plt.xlabel("Episode Step")
    ...
    def eval_src(self, num_games, render=False):
        ...
        for i in range(num_games):
            ...
            if self.if_normalize:
                state = self.running_mean(state[0])
            state = self.add_obs_noise(state)  # <-- Add noise (now applies to indices 100 and 226)
            done = False
            total_reward = 0
            step = 0
            while not done:
