...
from models.kalman_filter import LearnableKalmanFilter  # Existing module
from models.fft_filter import LearnableFFTFilter            # New FFT filter module
...
class ContSAC:
    def __init__(self, policy_config, value_config, env, device, log_dir="", running_mean=None,
                 ...
                 n_updates_per_train=1, max_steps=200, seed=None, use_kf=False, use_fft=False):
        if seed is not None:
    ...
    def reset_kf(self, init_state):
        """Resets the Kalman filter’s state and covariance."""
    ...
    def add_obs_noise(self, obs):
        """Adds Gaussian noise and then denoises using the selected method."""
        noisy_obs = obs + np.random.normal(0, self.noise_scale, size=obs.shape)
        if self.use_fft:
            noisy_obs_t = torch.tensor(noisy_obs, dtype=torch.float32, device=self.device)
            denoised = self.fft_filter(noisy_obs_t)
            return denoised.detach().cpu().numpy()
        elif self.use_kf:
            noisy_obs_t = torch.tensor(noisy_obs, dtype=torch.float32, device=self.device)
            if self.kf_state is None:
                self.reset_kf(noisy_obs)
            else:
                self.kf_state, self.kf_cov = self.kf(self.kf_state, self.kf_cov, noisy_obs_t)
            return self.kf_state.detach().cpu().numpy()
        else:
            return noisy_obs
    ...
    def train(self, num_games, deterministic=False):
        ...
        for i in range(num_games):
            ...
            if self.use_kf or self.use_fft:
                if self.use_kf:
                    self.reset_kf(state)
