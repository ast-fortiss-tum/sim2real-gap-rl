...
# TODO Discrete
class GailContSAC:
    def __init__(self, policy_config, value_config, env, device,expert, log_dir="latest_runs",running_mean = None,
                 memory_size=1e5, warmup_games=10, batch_size=64, lr=0.0001, gamma=0.99, tau=0.003, alpha=0.2,
                 ent_adj=False, target_update_interval=1, n_games_til_train=1, n_updates_per_train=1,max_steps = 200,eval_step = 0,noise_scale = 1):
        self.device = device
        self.gamma = gamma
        ...
        self.optimizer_discrim = torch.optim.Adam(self.discrim_net.parameters(), lr=0.0001)
    ...
    def train(self, num_games, deterministic=False):
        ...
        for i in range(num_games):
            ...
            while not done:
                ...
                total_reward += reward
                reward = self.expert_reward(state,next_state)
    ...
    def update_discriminator(self):
        for _ in range(1):
            state, a, r, next_state, d = self.ilmemory.sample()
            ...
            discrim_loss = self.discrim_criterion(g_o, torch.ones((state.shape[0], 1), device=self.device)) + \
    ...
    def expert_reward(self,state, next_state):
        state_action = torch.tensor(np.hstack([state, next_state]), dtype=dtype).to(self.device)
        with torch.no_grad():
            return -math.log(1e-8 + self.discrim_net(state_action  + gen_noise(self.noise_scale , state_action, self.device))[0].item())
