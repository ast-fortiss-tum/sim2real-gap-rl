...
source_broken = []
target_broken = [0]
env = BrokenJointEnv(gym.make('Ant-v2'), source_broken)
...
source_env = BrokenJointEnv(gym.make('Ant-v2'), source_broken)
target_env = BrokenJointEnv(gym.make('Ant-v2'),target_broken)
expert = DARC(policy_config, value_config, sa_config, sas_config, source_env, target_env, "cpu", ent_adj=True,
            n_updates_per_train=2,delta_r_scale=1,savefolder='none')
expert_path = 'Ant-v2-DARC2-1000steps_20230430_break0'
...
class Ppo:
    def __init__(self, N_S, N_A):
        ...
        self.scheduler_actor = torch.optim.lr_scheduler.StepLR(self.actor_optim,step_size=2, gamma=0.999)
        self.scheduler_critic = torch.optim.lr_scheduler.StepLR(self.critic_optim,step_size=2, gamma=0.999)
    def scheduler(self):
...
def cal_wt(s_states, s_actions, s_next_states):
    with torch.no_grad():
        ...
        sa_logits = expert.sa_classifier(sa_inputs.to('cpu'))
        sas_logits = expert.sas_adv_classifier(sas_inputs.to('cpu'))
...
def sample_expert_data(expert,memory_expert,env):
    ...
    for k in range(1):
        ...
        while not done:
            ...
            state = next_state
    # print(total_rewards)
    return memory_expert, total_rewards
...
def eval_target(env,num):
    ...
    for k in range(num):
        ...
        for _ in range(MAX_STEP):
            ...
            act = ppo.actor_net.choose_action_det(torch.from_numpy(np.array(s).astype(np.float32)).unsqueeze(0))[0]
...
memory_expert = sample_expert_data_target(expert,memory_expert,BrokenJointEnv(gym.make('Ant-v2'),[0]))
memory_expert = sample_expert_data_target(expert,memory_expert,BrokenJointEnv(gym.make('Ant-v2'),[]))
memory_expert = deque()
scheduler = torch.optim.lr_scheduler.StepLR(discriminator_opt, gamma=0.9999,step_size = 2)
r_exp = 0
for t in range(200):
    memory_expert,r = sample_expert_data(expert,memory_expert,source_env)
    r_exp += r
print('exp reward',r_exp/200)
for iter in range(Iter):
    ...
    steps = 0
    # memory_expert = deque()
    # memory_expert,_ = sample_expert_data(expert,memory_expert,source_env)
    
    s_origin = target_env.reset()
    s = nomalize(s_origin)
    score = 0
    ...
    while steps < Horizon:  # Horizen
        ...
        for _ in range(MAX_STEP):
            ...
            steps += 1
            act = ppo.actor_net.choose_action(torch.from_numpy(np.array(s).astype(np.float32)).unsqueeze(0))[0]
            s_, r, done, info = target_env.step(act)
            next_s_origin = s_
            s_ = nomalize(s_)
            mask = (1 - done) * 1
...
np.savetxt('savefile/Ant0eval_r_break_0_3_v5_to.txt',eval_reward_list)
np.savetxt('savefile/Ant0src_r_break_0_3_v5_to.txt',reward_src_list)
np.savetxt('savefile/Ant0acc_learner_v5_to.txt',acc_learner_list)
np.savetxt('savefile/Ant0acc_expert_v5_to.txt',acc_expert_list)
np.savetxt('savefile/Ant0acc_overall_v5_to.txt',overall_acc_list)
