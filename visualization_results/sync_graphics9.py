...
def analyze_run(poses_data, moving_data, survival_thresholds, safe_threshold):
    ...
    IMPORTANT: Instead of zipping data points, we now use the timestamps to decide which pose points are within
    a "going" interval.
    """
    # --- Load poses data ---
    ...
    # The logic: if a value is True then we consider that as the start of an interval.
    # We then scan forward until we find a False.
    # If no False is found, we take the interval end as the last pose timestamp.
    intervals = []
    ...
    while i < n_moving:
        if going_raw[i]:
            ...
            if j < n_moving:
                ...
                # If we never encounter a False, assume going lasts until the end of the run.
                end = pose_times[-1]
    ...
    # --- Helper function: Check if a timestamp is within any "going" interval ---
    ...
    # Determine a reference angle using the first pose that is during a "going" interval.
    ref_angle = None
    ...
    total_speed = 0.0
    speed_count = 0
    prev_going_idx = None  # index of the last pose that was within a "going" interval

    # --- Process each pose ---
    for i in range(len(poses)):
        # Only process poses whose timestamp falls in a "going" interval.
        ...
        # Compute speed from the previous "going" pose.
