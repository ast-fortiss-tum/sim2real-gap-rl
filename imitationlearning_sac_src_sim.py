...
parser.add_argument('--m-run', type=int, default=3,
                    help='name of Mujoco environement')


parser.add_argument('--classifier-node', type=int, default=64,
                    help='name of Mujoco environement')
parser.add_argument('--deterministic', type=int, default=1,
                    help='name of Mujoco environement')
parser.add_argument('--lr-dis', type=float, default=3e-4,
                    help='name of Mujoco environement')
parser.add_argument('--dis-batch-size', type=int, default=2048,
                    help='name of Mujoco environement')
parser.add_argument('--lower-bound', type=float, default=0.5,
                    help='name of Mujoco environement')
parser.add_argument('--upper-bound', type=float, default=1.5,
                    help='name of Mujoco environement')
...
sa_config = {
    "input_dim": [state_dim + action_dim],
    "architecture": [{"name": "linear1", "size": args.classifier_node},
                     {"name": "linear2", "size": 2}],
    "hidden_activation": "relu",
    "output_activation": "none"
}
...
for k in range(args.m_run):
    ...
    savepath = args.save_model.split(',')[k]
    ...
    for i_episode in tqdm(range(100000)):
        ...
        for t in range(args.episode_length):
            action = model.get_action(state, deterministic=args.deterministic)
    ...
    model.running_mean.fix = True
    IL = GailContSAC_SRC(policy_config, value_config,  source_env,target_env, device,expert_traj,model, ent_adj=True, n_updates_per_train=1,
