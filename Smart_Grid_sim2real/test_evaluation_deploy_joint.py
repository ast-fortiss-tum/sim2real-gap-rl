...
from models.darc import DARC_one
...
def parse_args():
    ...
    parser.add_argument("--training_seed", type=int, default=98,
    ...
    parser.add_argument("--pre_path", type=str,
                        default="/home/cubos98/Desktop/MA/DARAIL/saved_weights/saved_models_experiments_4/",
...
def construct_saved_model_path(args, prefix):
    """
    Dynamically construct a save model path based on experiment parameters.
    For non-broken (one-house) experiments, the name includes lin_src, variety,
    and either degree, capacity, or power limit. For broken experiments, it includes
    the break source.
    """
    # Format the fixed_start string
    fixed_start_str = args.fixed_start if args.fixed_start is not None else "None"
    # Start with the prefix, fixed start, learning rate, noise, and seed.
    path_name = f"{prefix}_test_run__fs{fixed_start_str}_lr0.0008_noise{args.noise}_seed{args.training_seed}_"
    
    if not args.broken:
        # For one-house experiments:
        path_name += f"lin_src{args.lin_src}_variety{args.variety_name}_"
        if args.variety_name in ['s', 'c', 'd'] or (args.variety_name and args.variety_name.startswith('v')):
            path_name += f"degree{args.degree}_"
        elif args.variety_name == 'lc':
            path_name += f"cap{args.capacity}_"
        elif args.variety_name == 'lp':
            path_name += f"p_lim{args.p_lim}_"
        path_name += "Smart_Grids"
    else:
        # For broken experiments:
        path_name += f"broken_{args.broken}_break_src{args.break_src}_Smart_Grids"
    
    # Combine with the base pre_path and return as a string.
    return str(pathlib.Path(args.pre_path) / path_name)
...
def main():
    ...
    # Build saved model paths dynamically using the helper function.
    saved_path_cont_sac = construct_saved_model_path(args, "ContSAC")
    saved_path_darc = construct_saved_model_path(args, "DARC")
