...
from models.src_gailsac_classifier_snips import GailContSAC_SRC_Classifier
...
parser.add_argument('--tau', type=float, default=0.995, metavar='G',
...
parser.add_argument('--normalize', type=int, default=1,
                    help='break which joint')
parser.add_argument('--use_pre_normalize', type=int, default=1,
                    help='break which joint')
...
parser.add_argument('--broken-p', type=float, default=1.0,
                    help='name of Mujoco environement')
parser.add_argument('--det-expert', type=int, default=1.0,
                    help='name of Mujoco environement')


parser.add_argument('--bs', type=int, default=256,
                    help='name of Mujoco environement')
parser.add_argument('--train-c', type=int, default=1,
                    help='name of Mujoco environement')
parser.add_argument('--model-size', type=int, default=32,
                    help='name of Mujoco environement')
parser.add_argument('--clipstate', type=int, default=10,
                    help='name of Mujoco environement')
parser.add_argument('--warmup', type=int, default=10,
                    help='name of Mujoco environement')
parser.add_argument('--policynet', type=int, default=256,
                    help='name of Mujoco environement')
parser.add_argument('--dis_update', type=int, default=50,
                    help='name of Mujoco environement')
...
sa_config = {
    "input_dim": [state_dim + action_dim],
    "architecture": [{"name": "linear1", "size": args.model_size},
                     {"name": "linear2", "size": 2}],
    "hidden_activation": "relu",
    "output_activation": "none"
}
...
running_state = ZFilter((state_dim,), clip=args.clipstate)
device = 'cpu'
...
for i_episode in tqdm(range(100000)):
    ...
    for t in range(args.episode_length):
        action = model.get_action(state, deterministic=args.det_expert)
        act_list.append(np.abs(action))
        ...
        expert_trajs[i_episode].append(np.hstack([state, next_state]))
    ...
    rewars_trajs.append(reward_episode)
    print(reward_episode)
print(np.mean(np.array(act_list),axis = 0))
    # print('Episode {}\t reward: {:.2f} \t step : {}'.format(i_episode, reward_episode,t))
print('eval on src',total_reward/(i_episode+1))
new_r = 0
tt = 0
for i in range(len(rewars_trajs)):
    if env_name != 'Reacher-v2':
        if rewars_trajs[i]>0.7*np.mean(rewars_trajs):
            tt += 1
            new_r += rewars_trajs[i]
            for t in expert_trajs[i]:
                expert_traj.append(t)
    else:
        if -rewars_trajs[i]>-1.3*np.mean(rewars_trajs):
            tt += 1
            new_r += rewars_trajs[i]
            for t in expert_trajs[i]:
                expert_traj.append(t)
...
for i_episode in tqdm(range(50)):
    ...
    if num_steps >= args.max_expert_state_num:
        break
    total_reward += reward_episode
print('eval on tgt',total_reward/(i_episode+1))
                        ...
                        max_steps = args.episode_length,\
                            running_mean=model.running_mean,\
                                warmup_games=args.warmup, \
                                    eval_step = 1,noise_scale = args.noise_scale,\
                                        update_freq = args.update_freq,
                                        if_normalize=args.normalize,\
                                    clamp_reward_lower_bound = args.lower_bound, clamp_reward_upper_bound = args.upper_bound,\
                                   reward_type = args.reward_type,train_c = args.train_c,model_size = args.model_size,dis_update_freq = args.dis_update)
                            ...
                            running_mean=running_state,\
                                warmup_games=args.warmup, \
                                    eval_step = 1,noise_scale = args.noise_scale,\
                                        ...
                                        if_normalize=args.normalize,\
                                    clamp_reward_lower_bound = args.lower_bound, clamp_reward_upper_bound = args.upper_bound,\
                                   reward_type = args.reward_type,train_c = args.train_c,model_size = args.model_size,dis_update_freq = args.dis_update,gp = args.gp)
