...
parser.add_argument('--m-run', type=int, default=3,
                    help='name of Mujoco environement')
parser.add_argument('--node', type=int, default=64,
                    help='name of Mujoco environement')
...
for k in range(args.m_run):
    ...
    sa_config = {
        "input_dim": [state_dim + action_dim],
        "architecture": [{"name": "linear1", "size": args.node},
                        {"name": "linear2", "size": 2}],
        "hidden_activation": "relu",
        "output_activation": "none"
    }
    ...
    model = DARC(policy_config, value_config, sa_config, sas_config, source_env, target_env, "cuda", ent_adj=True,
                n_updates_per_train=1,lr=args.lr,max_steps=args.max_steps,batch_size=256,savefolder=save_model_path,running_mean=running_state,if_normalize = True,warmup_games=50)
